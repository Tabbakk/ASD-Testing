		    ...
		    <li class="dropdown">
				...
				?>
						<li class="dropdown active">
							...
							<ul class="dropdown-menu">
							  <li class="active"><a href="../../therapist/stats/spdone.php">Pazienti</a></li>
...
?>
			<div class="page-header stats">
				<h2>Genera statistiche pazienti</h2>
					...
					<div class="col-sm-10 col-sm-offset-1">
						<div class="form-group col-sm-4 col-sm-offset-1">
							...
							<select class="form-control" name="group" id="group" required>
								<option value=""></option>
								<option value="ALL"<?php if(isset($_POST['group'])){if ($_POST['group'] == 'ALL'){echo(' selected="selected "');}} ?>>Tutti i gruppi</option>
							<?php
								foreach($_SESSION['group'] as $gid) {
									if($gid['id']!=-1){
										$sel='';
										if (isset($_POST['group'])){
											if ($_POST['group'] == $gid['id']) {$sel=' selected="selected" ';}
											else {$sel='';}
										}
...
<?php
	...
	if($_POST['group']=='ALL'){$group='';$gname=' di tutti i gruppi clinici';}
	else{
		if(is_numeric($_POST['group'])){
			$group=' and g.code='.$_POST['group'];
			$gname='del gruppo'.$_SESSION['group'][$_POST['group']]['name'];
		}
		else{
		$group='';
		$gname='di tutti i gruppi clinici';
		}
	}
?>
			<div class="page-header stats">
				<h2>Risultati  <?php echo($gname.' '.$period); ?></h2>
			</div>
			<div class="stats">
				<div class="row">
...
<!-- Test Quoziente di autismo -->
							<?php
								$sql= 'select count(idnumber) as num, avg(total) as avg from aq_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
								$sql2= 'select count(distinct patient) as pnum from aq_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
...
<!-- Test Attribuzione intenzioni -->
						<?php
							$sql= 'select count(idnumber) as num, avg(total) as avg from attint_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from attint_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
								if(!is_null($res['avg'])){
				...
				<!-- Test Basic Empathy Scale -->
						<?php
							$sql= 'select count(idnumber) as num, avg(ce) as avgce, avg(ae) as avgae from bes_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from bes_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
...
<!-- Test Attribuzione delle Emozioni -->
						<?php
							$sql= 'select count(idnumber) as num, avg(e1) as e1, avg(e2) as e2, avg(e3) as e3, avg(e4) as e4, avg(e5) as e5, avg(e6) as e6, avg(e7) as e7 from emotatt_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg and t.completed=1 '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from emotatt_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg and t.completed=1 '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
								if(!is_null($res['e1'])){
...
<!-- Test Eyes Task -->
						<?php
							$sql= 'select count(idnumber) as num, avg(total) as avg from eyestask_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from eyestask_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
								if(!is_null($res['avg'])){
...
<!-- Test Quoziente Empatia -->
						<?php
							$sql= 'select count(idnumber) as num, avg(ce) as ce, avg(ss) as ss, avg(ee) as ee, avg(total) as total from qe_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from qe_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
								if(!is_null($res['ce'])){
				...
				<!-- Test Situazioni Sociali -->
						<?php
							$sql= 'select count(idnumber) as num, avg(normb) as normb, avg(violation) as violation, avg(severity) as severity from socialsit_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from socialsit_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
				...
				<!-- Test Teoria della mente -->
						<?php
							$sql= 'select count(idnumber) as num, avg(total) as avg from tom_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg and t.completed=1 '.$interval.$group;
							$sql2= 'select count(distinct patient) as pnum from tom_c as t, clinicalg as g, patient as p where t.patient=p.id and g.code=p.cg and t.completed=1 '.$interval.$group;
							$result=$mysqli->query($sql);
							if($result){
								$res=$result->fetch_assoc();
								if(!is_null($res['avg'])){

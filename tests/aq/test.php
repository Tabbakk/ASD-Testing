<?php
	...
	if((int)($_GET['p'])){
		...
		$num=(int)($_GET['p']);
		if($num>1) {
			if(!isset($_POST['q'.($num-1)]) && !isset($_SESSION['aq_a']["'".($num-1)."'"])) {
				$_SESSION = array();
				session_destroy(); //distruggo tutte le sessioni
			...
			if(isset($_POST['q'.($num-1)])) {
				$_SESSION['aq_a']["'".($num-1)."'"]=$_POST['q'.($num-1)];
			}
		}
	}
	else {
...
<html>
	...
	<body>
		...
		<?php
			...
			if($active == 1) {
		?>
                  <li <?php if($testname=='aq'){echo('class="active"');} ?>><a href="../../tests/<?php echo($testname.'.php'); ?>"><?php echo($_SESSION['testnames'][$testname]); ?></a></li>
		<?php		
				}
			}
		...
		?>
			...
			<div class="page-header aqtest">
				<h4>Domanda numero <?php echo($num); ?></h4>
				<p class="question"><?php echo($_SESSION['aq']["'".$num."'"])?></p>
			</div>
			<div class="aqtest">
				<form id="aq" action="<?php echo($link); ?>" method="post">
					<div class="row">
						<div class="radio">
							<div class="col-sm-3 answ">
								<label>
									<input type="radio" value="A" name="q<?php echo($num); ?>" id="q<?php echo($num); ?>a" <?php if(isset($_SESSION['aq_a']["'".$num."'"])){if($_SESSION['aq_a']["'".$num."'"]=='A'){echo('checked="checked"');}} ?> required>
									Assolutamente d'accordo
								</label>
							</div>
							<div class="col-sm-3 answ">
								<label>
									<input type="radio" value="B" name="q<?php echo($num); ?>" id="q<?php echo($num); ?>b" <?php if(isset($_SESSION['aq_a']["'".$num."'"])){if($_SESSION['aq_a']["'".$num."'"]=='B'){echo('checked="checked"');}} ?> required>
									Parzialmente d'accordo
								</label>
							</div>
							<div class="col-sm-3 answ">
								<label>
									<input type="radio" value="C" name="q<?php echo($num); ?>" id="q<?php echo($num); ?>c" <?php if(isset($_SESSION['aq_a']["'".$num."'"])){if($_SESSION['aq_a']["'".$num."'"]=='C'){echo('checked="checked"');}} ?> required>
									Parzialmente in disaccordo
								</label>
							</div>
							<div class="col-sm-3 answ">
								<label>
									<input type="radio" value="D" name="q<?php echo($num); ?>" id="q<?php echo($num); ?>d" <?php if(isset($_SESSION['aq_a']["'".$num."'"])){if($_SESSION['aq_a']["'".$num."'"]=='D'){echo('checked="checked"');}} ?> required>
					...
					<div class="col-sm-6 col-sm-offset-3">	
						<div class="col-sm-4 col-sm-offset-2 button">

<?php
	...
	if(!isset($_SESSION['attint']['series'])) {
		$_SESSION = array();
	...
	if($_SESSION['attint']['series']==2 || $_SESSION['attint']['series']==3) {
		$nq = 12;
	}
	else {
		$nq = 14;
	...
	$_SESSION['attint_a']['q'.$nq]=$_POST['q'.$nq];
...
<html>
	...
	<body>
		...
		?>
			...
			<div class="page-header row attintresults">
		<?php
			$results = '';
			foreach ($_SESSION['attint_a'] as $risposta) {
				$results=$results.$risposta.',';
			}
			$results=rtrim($results, ",");

			$stmt = $mysqli->prepare('call attint_insert(?,?,?,?,?,@e)');
			$stmt->bind_param('siiii', $results, $_SESSION['attint']['series'], $_SESSION['user']['id'], $_SESSION['attint']['qtype'], $_SESSION['attint']['atype']);
		...
		<?php
			$_SESSION['testflag']['attint'][$_SESSION['attint']['series']]=1;
			}
			else {
		?>

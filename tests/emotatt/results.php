<?php
	...
	$_SESSION['emotatt_a']['58']=$_POST['q58'];
...
<html>
	...
	<body>
		...
		?>
			...
			<div class="page-header row emotattresults">

		<?php
			$resp = '';
			foreach ($_SESSION['emotatt_a'] as $dom) {
				$temp = str_replace(array('\\', "\0", "\n", "\r", "'", '"', "\x1a", ","), array(' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '),$dom);
				$resp = $resp.$temp.',';
			}
			$resp=rtrim($resp, ",");
			
			$stmt = $mysqli->prepare('call emotatt_insert(?,?,?,?,@e)');
			$stmt->bind_param('siii', $resp, $_SESSION['user']['id'], $_SESSION['emotatt']['qtype'], $_SESSION['emotatt']['atype']);
